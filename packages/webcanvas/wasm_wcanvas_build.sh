...
# TEST ONLY: exception catching disabled to shrink the wasm; not for release.
...
if [ "$MODE" = "pthread" ]; then
  ...
  sed "s|EMSDK:|$EMSDK/|g" ../wasm/wasm32.txt | \
    ...
    sed "s|, '-sEXPORTED_RUNTIME_METHODS=[^']*'||g" | \
    sed "s|'-sFORCE_FILESYSTEM=1'|'-sFILESYSTEM=0'|g" | \
    sed "s|'--bind'|'--bind', '--emit-tsd=thorvg.d.ts', '-sEXPORTED_FUNCTIONS=${EXPORTED_FUNCTIONS}', '-sEXPORTED_RUNTIME_METHODS=${EXPORTED_RUNTIME_METHODS}', '-sDISABLE_EXCEPTION_CATCHING=1', '-sDISABLE_EXCEPTION_THROWING=0', '-sALLOW_TABLE_GROWTH=1', '-sINITIAL_TABLE=128'|g" > /tmp/.wasm_webcanvas_cross.txt
fi
