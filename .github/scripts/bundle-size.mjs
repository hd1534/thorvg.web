...
// "🟥 +211 (+0.02%)" / "🟩 -1,204 (-0.10%)" / "0"
function deltaCell(base, pr, withPct = true) {
  const diff = pr - base;
  if (diff === 0) return '0';
  return `${diff > 0 ? UP : DOWN} ${signed(diff)}${withPct ? pct(base, diff) : ''}`;
}
  ...
  for (const { preset, m, p } of rows) {
    ...
    for (const mod of mods) {
      if (!sizeOrNull(m?.bundles?.[mod]) || !sizeOrNull(p?.bundles?.[mod]) || !sizeOrNull(m?.wasm) || !sizeOrNull(p?.wasm)) {
        modules.push(`| ${preset} | ${mod} | n/a | n/a | n/a | n/a | n/a | n/a |`);
        continue;
      }
      const tm = total(m, mod);
      const tp = total(p, mod);
      modules.push(
        `| ${preset} | ${mod} | ${fmt(tm.raw)} | ${fmt(tp.raw)} | ${deltaCell(tm.raw, tp.raw)} ` +
    ...
    for (const [kind, fm, fp] of entries) {
      ...
      if (!sm || !sp) {
        files.push(`| ${preset} | ${file} | ${sm ? fmt(sm.raw) : 'n/a'} | ${sp ? fmt(sp.raw) : 'n/a'} | n/a | ${sm ? fmt(sm.gzip) : 'n/a'} | ${sp ? fmt(sp.gzip) : 'n/a'} |`);
        continue;
      }
      files.push(`| ${preset} | ${file} | ${fmt(sm.raw)} | ${fmt(sp.raw)} | ${deltaCell(sm.raw, sp.raw)} | ${fmt(sm.gzip)} | ${fmt(sp.gzip)} |`);
    }
  }
...
function renderPackage(name, sizes) {
  ...
  const details = [
    ...
    '',
    `Total = thorvg.wasm + ${ok[0]?.mod ?? 'esm'} bundle.`,
    '',
    ...table,
    '',
